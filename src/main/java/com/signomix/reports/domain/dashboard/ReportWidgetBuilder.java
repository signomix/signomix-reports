...
public class ReportWidgetBuilder extends AbstractWidgetBuilder {
    ...
    public String buildContent(User user, Widget widget, String timeZone) {
        ...
        StringBuilder content = new StringBuilder(
            "<div class=\"overflow-scroll\"><table class=\"table table.sm\">\n<thead>\n<tr>\n"
        );
        content.append(
        ...
        for (int i = 0; i < result.headers.get(0).columns.size(); i++) {
            content
                .append("<th scope=\"col\" class=\"text-end\">")
                .append(result.headers.get(0).columns.get(i))
                .append("</th>\n");
        }
        content.append("</tr>\n</thead>\n<tbody>\n");
        if (result.datasets.size() > 0) {
            for (int i = 0; i < result.datasets.get(0).data.size(); i++) {
                ...
                content
                    .append("<tr>\n<td>")
                    .append(
                        getTimestampFormatted(datasetRow.timestamp, timeZone)
                    )
                    .append("</td>\n");
                for (int j = 0; j < datasetRow.values.size(); j++) {
                    String valueText;
                    try {
                        valueText = getValueFormatted(
                            (double) datasetRow.values.get(j),
                            widget.rounding
                        );
                    } catch (Exception e) {
