...
public abstract class AbstractWidgetBuilder {
    ...
    protected static final String ERR_QUERY_MESSAGE = "<div class=\"widget-error\">error parsing query</div>\n";
    protected static final String ERR_DATA_MESSAGE = "<div class=\"widget-no-data\">no data</div>\n";
    ...
    public abstract String buildContent(User user, Widget widget, String timeZone);
    ...
    protected Measurement getSingleValue(User user, DataQuery query) {
        ...
        if (result.datasets.size() == 0 || result.datasets.get(0).data.size() == 0) return null;
    ...
    protected String getIconText(double value, Long timestamp, String alertRule, String[] icons) {
        String[] iconNames = {"bi-emoji-smile", "bi-emoji-neutral", "bi-emoji-frown", "bi-emoji-expressionless"};
        for (int i = 0; i < icons.length; i++) {
            if (!icons[i].trim().isEmpty()) iconNames[i] = icons[i].trim();
        }
        int alertLevel = alertLevelService.getAlertLevel(alertRule, value, timestamp);
        int index = alertLevel >= 0 && alertLevel < iconNames.length ? alertLevel : 3;
